                              ...
                              unsigned size)
{
    ...
    case A_MISC:
        /*
         * Bit 8 is the chip-select for the SPI NOR flash behind SPI
         * controller 0 (0 = assert/select, 1 = deassert/release).
         * The remaining control bits (CLCD etc) are not modelled.
         */
        if (value & ~BIT(8)) {
            qemu_log_mask(LOG_UNIMP,
        ...
        s->misc = value;
        qemu_set_irq(s->spi_cs, extract32(value, 8, 1));
        break;
    case A_CLK1HZ:
        now = qemu_clock_get_ns(QEMU_CLOCK_VIRTUAL);
...
static void mps2_fpgaio_init(Object *obj)
{
    ...
    qdev_init_gpio_out_named(DEVICE(obj), &s->spi_cs, "spi-cs", 1);
}
